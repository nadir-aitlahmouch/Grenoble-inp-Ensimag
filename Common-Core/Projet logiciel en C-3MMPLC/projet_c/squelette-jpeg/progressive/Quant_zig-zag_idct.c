...
void quantification_inverse(struct jpeg_desc *jdesc, int16_t *tab_frequences,
                             uint8_t i, uint8_t indice_debut, uint8_t indice_fin){
  ...
  uint8_t *quant_table = get_quantization_table(jdesc, quant_index_table);
    for (uint8_t i = indice_debut; i < indice_fin + 1; i++) {
...
void zigzag_inverse(int16_t *tab_frequences, uint8_t indice_debut, uint8_t indice_fin){
    ...
    int16_t temp[indice_fin + 1 - indice_debut] = {0};
    for (uint8_t i = indice_debut; i < indice_fin + 1 ; i++) {
