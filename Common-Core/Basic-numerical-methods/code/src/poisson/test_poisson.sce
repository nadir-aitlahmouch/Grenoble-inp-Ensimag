...
function plot_error(F, Ref, Psi)
    ...
    xset("wdim", 1900,1200)
    // Plotter les fonctions
    subplot(2,2,1)
    colorbar(min(F),max(F))
    surf(F)
    xtitle("Fonction analytique testée")
    subplot(2,2,2)
    colorbar(min(Ref),max(Ref))
    surf(Ref)
    xtitle("Solution analytique de référence")
    subplot(2,2,3)
    colorbar(min(Psi),max(Psi))
    surf(Psi)
    xtitle("Solution obtenue avec le solveur de Poisson FFT")
    subplot(2,2,4)
    Erreur = abs(Ref-Psi)
    colorbar(min(Erreur),max(Erreur))
    surf(Erreur)
    xtitle("Erreur absolue commise dans le rapport")
