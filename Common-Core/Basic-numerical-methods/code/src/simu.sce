...
function plot_isocontours(W, figname)
    ...
    colorbar(min(W), max(W), cmm)
    Sgrayplot(X,Y,W', colminmax=cmm)
    contour2d(X,Y,W', -36:6:36) //-70:10:70 pour le deuxième paramétrage
    xlabel("x")
    ylabel("y")
...
function plot_champs(nom_figure)
    ...
    for x = 1:Nx/4
        ...
        for y = 1:Ny/4
            ...
            Uy2(x,y) = Ux((x-1)*4+1, (y-1)*4+1)
            Intensity(x,y) = sqrt(Ux2(x,y)^2 + Uy2(x,y)^2)
        end
    end
    title("Champs des vitesses")
    champ1(X, Y, Ux', Uy')
    colorbar(min(Intensity), max(Intensity))
    xlabel("x")
    ylabel("y")
    
    nom_figure = sprintf("Champs_%f.png", t)
    x2png(fig, nom_figure)
endfunction
